...
in the middle of a run, on the history that run has produced so far, and the
script polls the cell until the program's own preconditions hold. The address it
prints is `VITE_POLICY_PUBKEY`.

**Which is why a show is two runs and not one.** A policy cannot exist before
fourteen covered days and a waiting period, and by then a single 29-day fixture
has almost no drought left to insure — five to seven dry days inside the window,
against the eighteen the fixture actually contains. So the demo plays
`normal.json` first, buys the policy on that wet history, and then plays
`drought.json`: the second run lands on days the pool has not lived yet, and the
whole eighteen-day spell falls inside the policy's window. A run picks each
sensor's counter up where the table left it, the way a device resumes its log
after a restart, so the same fixture can be played again without emptying the
database first.
